...
export function MaterialArchive() {
	...
	const stats = [
		{
			label: "RPP",
			count: materials.filter((m) => m.type === "RPP").length,
			icon: FileText,
			color: "bg-blue-500",
			text: "text-blue-600",
			bg: "bg-blue-50",
		...
		{
			label: "PPT",
			count: materials.filter((m) => m.type === "PPT").length,
			icon: Presentation,
			color: "bg-purple-500",
		...
		{
			label: "Soal",
			count: materials.filter((m) => m.type === "QUESTIONS").length,
			icon: ClipboardList,
			color: "bg-green-500",
			text: "text-green-600",
			bg: "bg-green-50",
		...
		{
			label: "LKPD",
			count: materials.filter((m) => m.type === "LKPD").length,
			icon: BookOpen,
			color: "bg-orange-500",
			...
			setMaterials([
				{
					...
					metadata: {
						...
						topic: "Hukum Newton tentang Gerak",
					},
				},
				{
					id: "2",
					title: "PPT Integral Tak Tentu",
					type: "PPT",
	...
	const handleCreateExam = async () => {
		...
		try {
			const exam = await scoringApi.createExamFromMaterial(
				selectedMaterial.id,
			);
			toast({
				title: "Ujian berhasil dibuat!",
				description: `Ujian "${exam.title}" berhasil dibuat dari soal.`,
			});
			setDetailOpen(false);
	...
	const getTopic = (material: Material) => {
		...
		return (
			...
			material.title?.replace(/^(RPP|PPT|Soal|LKPD)\s*-?\s*/i, "") ||
			"-"
		);
	};
									...
									>
										<TableCell>
											<div className="flex items-start gap-3">
												<div
													className={`p-2 rounded-lg ${getIconBg(material.type)} shrink-0 mt-0.5`}
												>
										...
										<TableCell className="text-center">
											<Badge
												variant="secondary"
												className="border-gray-400 rounded-full bg-white text-black font-normal hover:bg-white"
											>
										...
										<TableCell className="text-center">
											<Badge
												variant="outline"
												className={`${material.isPublished ? "bg-green-50 text-green-700 border-green-200" : "bg-gray-100 text-gray-600 border-gray-200"} font-normal`}
											>
			...
			<Dialog open={detailOpen} onOpenChange={setDetailOpen}>
				<DialogContent className="sm:max-w-[500px] p-0">
					<DialogHeader className="p-6 pb-4">
						<div className="flex items-start gap-3">
							<div
								className={`p-3 rounded-xl ${getIconBg(selectedMaterial?.type || "")} shrink-0`}
							>
					...
					<div className="px-6 pb-6 space-y-4">
						...
						<div className="grid grid-cols-2 gap-4">
							...
							<div className="bg-gray-50 rounded-xl p-4">
								...
								<p className="font-medium text-gray-900">
									{formatKurikulum(
										selectedMaterial?.metadata?.kurikulum,
									)}
								</p>
							</div>
						</div>
						...
						<div className="flex gap-3 pt-2">
							...
							{selectedMaterial?.type === "QUESTIONS" && (
								<Button
									className="flex-1 bg-gradient-to-r from-[#6E2CF4] to-[#0093E9] hover:opacity-90 text-white"
