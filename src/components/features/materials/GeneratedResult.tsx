...
import { Download, ExternalLink, Loader2, FileText, Presentation, BookOpen, ClipboardList } from "lucide-react";
...
export function GeneratedResult({ result, type }: GeneratedResultProps) {
  ...
  const previewUrl = result?.previewUrl || result?.preview_url || result?.data?.previewUrl || result?.data?.preview_url;
  const fileUrl = result?.fileUrl || result?.url || result?.data?.fileUrl || result?.data?.url;
    ...
    <div className="flex flex-col h-full overflow-hidden bg-white rounded-xl">
      ...
      <div className="p-3 flex items-center justify-between border-b bg-gray-50/50">
        ...
        <div className="flex gap-2">
          {previewUrl && (
            <Button size="sm" variant="outline" className="text-xs h-8" onClick={() => window.open(previewUrl, '_blank')}>
      ...
      {/* Preview Content */}
      <div className="flex-1 overflow-hidden relative bg-gray-100" style={{ minHeight: '500px' }}>
        {previewUrl ? (
          <>
            {isLoading && (
              <div className="absolute inset-0 flex items-center justify-center bg-gray-100 z-10">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-gray-500 font-medium">Memuat preview...</p>
                </div>
              </div>
            )}
            <iframe
          ...
          <div className="flex flex-col items-center justify-center h-full min-h-[400px] p-6">
            <div className="bg-green-50 border border-green-200 rounded-xl p-6 text-center max-w-md">
              ...
              <p className="text-green-600 text-sm mb-4">
                File berhasil di-generate. Klik "Download" untuk mengunduh file.
              </p>
              {fileUrl && (
                <Button
                  onClick={handleDownload}
